...
/*
문제 : 격자판에 3개의 조각이 있다. 세 조각들을 움직여서 세 조각을 분리할 수 있는지 구하고, 가능하다면 모든 조각을 분리하기 위해
필요한 최소 이동 횟수를 구한다.
세 조각의 좌표는 X와 Y좌표 모두 0 이상 9 이하의 정수이다. 세 조각이 분리된 것은 각 조각을 둘러싸는 직사각형들이
서로 겹치지 않을 때이다.

해결 방법 : 첫 번째 조각은 가만히 두고 다른 조각들의 상대적인 위치를 처음에 대비해 움직여가며 BFS를 한다.
다른 두 조각의 처음 위치와의 X와 Y좌표 차이를 변수로 두고 플러드필을 하면 된다.
첫 번째 조각을 움직이는 경우는 두 조각의 좌표를 동시에 이동하는 것으로 구현 가능하다.
각 조각들을 둘러싸는 직사각형이 겹치지 않는지는 좌표의 최댓값과 최솟값을 비교하는 방식으로 구할 수 있다.
...
cxl = INF, cxh = -INF, cyl = INF, cyh = -INF;
...
int bfs(int bx, int by, int cx, int cy) {
    int d = 0, valid;
    if (boxcheck(bx, by, cx, cy)) return 0;
    vis[bx][by][cx][cy] = 1;
...
int main(void) {
    //freopen("E:\\PS\\Olympiad\\USA\\2_Silver\\2012_04\\unlock_silver\\11.in", "r", stdin);
