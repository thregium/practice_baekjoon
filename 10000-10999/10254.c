...
/*
문제 : 좌표평면상에 N(N <= 200000)개의 점이 주어진다. 각 점들의 좌표는 절댓값 10^7 이하의 정수이다.
이때, 가장 먼 두 점의 쌍을 구한다.

해결 방법 : 볼록 껍질을 만든 뒤 회전하는 캘리퍼스 알고리즘을 사용하면 된다.
...
point pts[103000 * 2], cvh[103000 * 2];
long long sx, sy;
point res1, res2;
...
long long rotating_calipers(point* cvh, int n) {
    ...
    int lower = 0, upper = 0, baseline = 0, finished = 0;
    long long res = 0;
