...
/*
문제 : N(N <= 100000)개의 수로 이루어진 수열이 있고, 이 수열의 원소들은 각각 10억 이하의 자연수이다.
Q(Q <= 100000)개의 다음 쿼리에 응답한다.
L, R이 주어질 때 L번째에서 R번째 사이 수들 가운데 합이 0인 가장 긴 구간을 구한다.

해결 방법 : 먼저, 각 수열의 원소들에 대해 누적 합을 구한다.
그러면 L번째에서 R번째 사이의 수들 가운데 가장 멀리 떨어진 같은 값을 찾는 문제가 된다. 이는 아래 방법을 이용해 구할 수 있다.

K개의 각 수별로 현재 범위의 수들을 덱을 이용해 관리한다. 범위가 추가되면 덱의 해당 방향에 수들을 추가하고
...
int a2[103000], a[103000], qr[103000][4], r[103000];//, stt[103000], real[103000];
list<int> dq[1048576];
int cnt[1048576], dqs[512];
...
int main(void) {
    ...
    for (int i = 0; i < m; i++) {
        ...
        /*
        qr[i][0] = (rand() * 32768 + rand()) % k + 1;
        srand(qr[i][0] + rand());
        qr[i][1] = (rand() * 32768 + rand()) % k + 1;
